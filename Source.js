...
function getSelectedUserProxy(userSocks5, request) {
	...
	// مقدار پیش‌فرض را -1 می‌گذاریم تا مسیرهای بدون loc را تشخیص دهیم
	let idx = -1; 
	if (request) {
		try {
			const url = new URL(request.url);
	...
	// اگر ایندکس -1 ماند، یعنی مسیر دقیقاً همان Base Path (دایرکت) است
	if (idx === -1) return "";
...
<div id="info-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 opacity-0 pointer-events-none transition-all duration-300 ease-out">
	<div class="w-full max-w-md bg-white dark:bg-amoled-card border border-purple-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col">
		...
		<div class="text-xs text-gray-600 dark:text-gray-300 mb-4 leading-relaxed font-medium text-justify space-y-2">
			<p>
				با کمال تأسف به اطلاع می‌رسانیم که مخزن گیت‌هاب پروژه زئوس به دلیل گزارش‌های کذب و مغرضانه برخی از افراد سودجو و <strong>فروشندگان کانفیگ</strong> مسدود شده است.
			</p>
			<p>
			...
			<p>
				پروژه زئوس همواره بر پایه شفافیت مطلق بنا شده است. برای اثبات این موضوع، سورس‌کد کامل پنل در دسترس شماست. لطفاً به شایعات توجه نکنید و برای اطمینان خاطر، کدها را مستقلاً با ابزارهای هوش مصنوعی بررسی کنید تا به سلامت و امنیت کامل پروژه پی ببرید.
			</p>
					...
					if (user.user_proxy_iata) {
						...
						locBadge = proxyList.map(item => {
							...
							if (cachedFlag && typeof cachedFlag === 'string' && /^[a-zA-Z]{2}$/.test(cachedFlag) && typeof getFlagEmoji === 'function') {
								return '<span title="پـروکـسـی اختصاصی" class="' + flagSizeClass + ' leading-none px-0.5 drop-shadow-[0_0_2px_rgba(0,0,0,0.3)] dark:drop-shadow-[0_0_2px_rgba(255,255,255,0.3)]">' + getFlagEmoji(cachedFlag) + '</span>';
							} else {
